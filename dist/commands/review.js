...
const trae_executor_1 = require("../utils/trae-executor");
const branch_detection_1 = require("../utils/branch-detection");
const executor = new trae_executor_1.TraeExecutor();
async function review(args, isAdversarial = false) {
    ...
    if (!args.includes('--base')) {
        ...
        baseBranch = await (0, branch_detection_1.detectBaseBranch)();
    }
    console.log(`📊 分析变更大小...`);
    const estimate = await (0, branch_detection_1.estimateReviewSize)(baseBranch);
    console.log((0, branch_detection_1.formatEstimate)(estimate));
    if (!config.background && estimate.recommendation.useBackground) {
        console.log('💡 自动启用后台模式 (使用 --background 覆盖)');
        config.background = true;
    }
    console.log(`\n获取与 ${baseBranch} 的差异...`);
    const diff = await (0, utils_1.getGitDiff)(baseBranch);
    if (!diff.trim()) {
