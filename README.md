...
在 Claude Code 和 OpenCode 中无缝使用字节 Trae Agent (trae-cli) 的插件。支持任务委托、代码审查、会话管理、ACP 协议通信，实现与原生功能一致的体验。
...
| OpenCode 工具 | OpenCode 原生工具集成（7 个 Bun 工具 + 10 个命令） | `.opencode/tools/`, `.opencode/commands/` |
| OpenCode 插件 | OpenCode 事件钩子（session lifecycle） | `.opencode/plugins/trae-hooks.ts` |
...
本插件支持作为 OpenCode 插件加载，提供 7 个 Bun 工具和 10 个命令定义。
...
#### OpenCode 工具定义（`.opencode/tools/`）

| 工具文件 | 对应 CLI 命令 | 功能 |
|----------|--------------|------|
| `trae-setup.ts` | `setup` | 检查 trae-cli 安装和认证状态 |
| `trae-run.ts` | `run` | 执行自然语言任务 |
| `trae-review.ts` | `review` / `adversarial-review` | 代码审查 |
| `trae-sessions.ts` | `sessions` | 会话管理（list/detail/conversation/tools/context/recent/find/delete/delete-smoke） |
| `trae-acp.ts` | `acp` | ACP 协议管理（start/stop/status/agents/run/stream） |
| `traе-status.ts` | `status` | 后台任务状态 |
| `traе-result.ts` | `result` | 获取后台任务输出 |
| `traе-cancel.ts` | `cancel` | 取消后台任务 |
| `traе-rescue.ts` | `rescue` | 故障恢复诊断 |

#### OpenCode 命令（`.opencode/commands/`）

提供 10 个命令 Markdown 文件，供 OpenCode 的 Agent 直接调用：
`trae-setup.md`, `traе-run.md`, `traе-review.md`, `traе-adversarial-review.md`,
`traе-sessions.md`, `traе-acp.md`, `traе-setup.md`, `traе-status.md`,
`traе-result.md`, `traе-cancel.md`, `traе-rescue.md`

#### OpenCode 插件钩子（`.opencode/plugins/traе-hooks.ts`）

将 Claude Code 生命周期钩子映射为 OpenCode 事件：
...
| 会话恢复 | `--resume [ID]` / `--resume=<ID>` | 恢复之前的会话继续执行 |
| Worktree 隔离 | `--worktree [NAME]` / `--worktree=<NAME>` | 在隔离的 git worktree 中执行，不影响主分支 |
...
| 参数 | 标准语法 | 等号语法 |
|------|---------|---------|
| 恢复会话 | `--resume [ID]` | `--resume=<ID>` 或 `--resume=AUTO` |
| 工作树隔离 | `--worktree [NAME]` | `--worktree=<NAME>` |
...
| `delete-smoke` | 批量删除标题或 ID 包含 "smoke" 的会话 | `/trae:sessions delete-smoke` |
...
内部命令，由 `hooks/hooks.json` 中配置的生命周期钩子自动触发，一般用户无需手动调用。

**支持的子命令**:

| 子命令 | 触发场景 | 对应脚本 |
|--------|---------|---------|
| `session-start` | Claude Code 会话启动时 | `scripts/session-lifecycle-hook.mjs` |
| `session-end` | Claude Code 会话结束时 | `scripts/session-lifecycle-hook.mjs` |
| `stop-gate` | 用户停止操作时 | `scripts/stop-review-gate-hook.mjs` |

**内部实现**:
- 由 `hooks.json` 中配置的钩子自动触发
- 使用 `node` 直接执行 `.mjs` 脚本
- `session-start`: 检查 trae-cli 安装和配置状态，提醒后台任务
- `session-end`: 清理过期日志文件（7天以上），提醒运行中的后台任务
- `stop-gate`: 检测未提交的代码变更和运行中的后台任务，建议执行代码审查
...
/trae:rescue [--context TEXT]  # 故障恢复
/trae:hooks <hook-type>  # 钩子处理（内部使用）
```
...
# 添加新工具: 在 .opencode/tools/ 下创建 .ts 文件，使用 @opencode-ai/plugin 的 tool() API
# 添加新命令: 在 .opencode/commands/ 下创建 .md 文件，遵循 Frontmatter + 说明格式
# 添加新钩子: 在 .opencode/plugins/trae-hooks.ts 中扩展事件处理
```

#### OpenCode 开发注意事项

- OpenCode 工具使用 **Bun** 运行时（`.opencode/package.json` 使用 `bun` 执行）
- 工具通过 `import { $ } from "bun"` 调用 CLI，路径基于 `import.meta.dir` 解析到项目根目录
- 工具文件导出为 `export default tool({...})` 格式，文件名即为工具名
- 命令文件使用 Frontmatter（`description:`）定义，内容为 Agent 可用的指令文本
- 修改 `.opencode/tools/*.ts` 后**不需要编译**，Bun 直接执行 TypeScript
- 修改 `src/*.ts` 后必须 `npm run build`，因为 OpenCode 工具调用的是 `dist/index.js`
